...
| `<div id="editor">`          | Core editor container with `#viewport`, `#sizer`, virtual rows container `#rows`, decoupled caret, and the VS Code-style `#sticky-symbol` declaration header that shows the enclosing function or type while scrolling.                                                                                                                                                                                                |
...
| `web/src/renderer.js`  | `measure()`, `layout()`, `render()`, `paint()`, `toggleWordWrap()`, `toggleLineNumbers()`, on-demand chunk fetching, and the sticky enclosing-symbol header.                                                                        |
| `web/src/tabs.js`      | `openFile()`, `closeTab()`, `switchTab()`, `drawTabs()`, `drawCrumbs()`, `showImage()`, `reloadOpenTabs()` (in-place tab refresh on reindex), `reopenClosedTab()` (Alt+Shift+T), and outline warming for sticky symbols.          |
