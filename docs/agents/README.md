...
| `web/src/tree.js`      | `drawTree()`, `fileKind()`, `expandDir()`, `revealDir()`, `revealFile()`, explorer tree click handlers. Auto-reveal: `syncTreeSelection()` (called whenever the active document changes), `flushPendingReveal()`, `autoRevealOn()`, `setAutoReveal()`. |
...
| `web/src/panels.js`    | `showPanel()`, `toggleSidebar()` (the single entry point for sidebar visibility; replays any deferred reveal), reindex trigger, draggable sidebar resizer.                                   |
