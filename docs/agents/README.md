...
| `<div id="diffview">`        | Git diff over `#viewport` for tabs with a diff layout active (`Split` / `Unified` switch, `Mod+D` toggles). Not virtualized: a single file's diff is small. |
| `<div id="imgview">`         | Image viewer over `#viewport` for image tabs (png/jpg/svg/...), holding a single reused `<img id="img">` served from `/api/raw`. Driven by `syncImage()` in `web/src/image.js`, like the Markdown and diff overlays. |
...
| `web/src/tabs.js`      | `openFile()`, `closeTab()`, `switchTab()`, `drawTabs()`, `drawCrumbs()`, `reloadOpenTabs()` (in-place tab refresh on reindex). Image paths become lightweight `{ image: true }` tabs (no lines/gutter/LSP/outline). `reopenClosedTab()` (Alt+Shift+T) reopens from stack of last 20 closed files. |
| `web/src/image.js`     | `isImage()`, `syncImage()`: single reused `<img>` viewer over `#viewport` for image tabs. Same tab lifecycle (switch, `Alt+W`, cross, `Ctrl+Tab`, `Alt+1…9`) as code, none of its language machinery. |
