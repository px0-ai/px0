...
| `<div id="minimap">`         | Minimap canvas and `#minimap-slider`, beside `#viewport` while the code view shows (`#editor.mm-on` narrows the viewport by `--mm-w`). Hidden for diffs, Markdown previews, and editors under 600px. |
...
| `web/src/renderer.js`  | `measure()`, `layout()`, `render()`, `paint()`, `toggleWordWrap()`, `toggleLineNumbers()`, on-demand chunk fetching, `setAfterPaint()` hook.                                                |
| `web/src/minimap.js`   | `initMinimap()`, `toggleMinimap()` (Alt+K), `runsOf()`: canvas minimap drawn after each paint from already-fetched lines, slider drag, click to jump, wheel forwarding.                     |
...
| `web/src/find.js`      | `openFind()`, `clearFind()`, `runFind()`, `jumpToHit()`, whole-file match dots in `#minimap-hits` (Ctrl+F).                                                                                                         |
