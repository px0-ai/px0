...
| `web/src/history.js`   | `pushHistory()`, `go()`: jump history back/forward (Alt+Left, Alt+Right). `touchRecent()`: most recently opened files (`S.recent`), updated by `openFile()` and `switchTab()`.            |
...
| `web/src/palette.js`   | `openPalette()`, `refreshPalette()`, `COMMANDS`, fuzzy file/symbol/command finder. File mode lists the active file first (while typing, only if it matches, with selection on the next match); an empty query adds the 5 most recent others; the index follows, deduplicated. |
