...
The minimap ([`web/src/minimap.js`](../../web/src/minimap.js)) draws with these same tokens, read from the root's computed style and re-read whenever `data-theme` changes, plus `--bg`, `--fg`, `--cur`, `--mark-active` (find matches), `--gi`/`--accent`/`--gd` (git marks) and `--bg4` (its slider). A theme needs nothing extra for it.
