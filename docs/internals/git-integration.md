...
1. **Intraline highlights.** For each equal-sized replacement run, `intraline.js` runs a bounded Myers diff over lossless whitespace, identifier/number, and punctuation tokens. It preserves every unchanged range and marks each added or removed range; short changed identifiers and numbers receive a second character-level pass. Uneven line-to-block replacements and lines over 2,048 code points or 512 tokens keep the existing whole-line treatment, bounding both time and memory. The annotation is attached to the parsed rows, so split and unified layouts render the same segments without another comparison or API call.
