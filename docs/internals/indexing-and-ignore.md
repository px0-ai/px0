...
Remote SSH indexes preserve the same contract after the initial remote listing: `.git`, `.hg`, and `.svn` descendants are suppressed, `.gitignore` rules are applied to remote paths, ignored entries stay visible in the explorer but are omitted from fuzzy find and normal workspace search, and an exact find-in-file search can still read an opened ignored remote file through the remote reader.
