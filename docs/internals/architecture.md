...
| `/api/diff`           | `GET`  | Unified diff of working tree vs. `HEAD` (`?path=...`) with size-budgeted parallel syntax-highlighted lines | JSON (`{path, diff, diffLines?, diffHighlightSkipped, available}`) |
