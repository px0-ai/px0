...
| **GitHub Copilot CLI** | `gpt-5.4-mini` | `copilot --allow-all-tools --no-ask-user -s --model gpt-5.4-mini -p {prompt}` |
...
- **Agent: Harness** (`agent.harness`): The default CLI tool to execute (`claude`, `gemini`, `cursor-agent`, `agy`, `opencode`, `codex`, `aider`, `goose`, `copilot`).
