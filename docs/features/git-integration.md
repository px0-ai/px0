...
- **Expandable Hunk Context**: The unchanged lines git skips between hunks (and around the first and last hunk) collapse into a GitHub-style expand chevron row. Click a side chevron to reveal 20 more lines at a time, or the middle chevron to open a larger run in one go (up to 500 lines). Expanded lines are syntax-highlighted, carry old/new line numbers on both sides, and can be selected and sent to the agent like any diff row. Expansions reset automatically whenever the diff itself refreshes.
