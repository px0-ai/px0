...
- **File Explorer vs. Git Changes Toggle**: A dedicated segmented toggle in the sidebar header allows you to switch between the full project directory tree and the Git changes view. In Git changes mode, px0 collapses untouched folders and presents only files with uncommitted additions, modifications, or deletions. Folders you collapse in this view stay collapsed while status updates arrive (staging a file, an agent save, a terminal `git` command). A collapsed folder opens again when you click it, when px0 reveals a file inside it, or when it no longer contains changes.
