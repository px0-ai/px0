...
- Explorer Follows the Active File: Open a file any way you like (quick open, a search hit, go-to-definition, a tab, jump history) and the sidebar expands to it and selects it, so you never lose your place in the tree. It only scrolls when the row is off screen, defers while the sidebar is hidden, and is switched off with `Toggle Auto-Reveal Active File in Explorer` in the command palette.
