...
def test_retrieve_dispatches_correctly(tmp_home, monkeypatch):
    called = []
    monkeypatch.setattr(retrieval, "_qmd_retrieve",
                        lambda *a, **k: called.append("qmd") or [])
...
def test_local_only_returns_k_results_when_work_rows_rank_higher(tmp_home):
    ...
    base = retrieval.brain_path(tmp_home, {})
...
def test_a_work_passage_from_qmd_is_actually_withheld(tmp_home, monkeypatch):
    ...
    got = retrieval.retrieve(
        tmp_home, {"retrieval": {"backend": "qmd"}}, "margin", k=5, local_only=True
    )
...
def test_qmd_work_passages_are_still_reachable_when_asked_for(tmp_home, monkeypatch):
    ...
    got = retrieval.retrieve(
        tmp_home, {"retrieval": {"backend": "qmd"}}, "internal", k=5, local_only=False
    )
